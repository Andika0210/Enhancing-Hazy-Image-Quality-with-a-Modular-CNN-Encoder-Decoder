...
from tensorflow.keras.layers import (
    ...
    LayerNormalization, Dense, AveragePooling2D, UpSampling2D, Reshape, Lambda
)
...
# === Helper: gMLP-style block (menggunakan pooling untuk kurangi seq_len agar efisien) ===
def gmlp_block(x, pool_size=4, mlp_ratio=2, name=None):
    """
    x: feature map (B, H, W, C)
    pool_size: integer untuk downsample posisi sebelum token-mixing (mengurangi seq_len)
    mlp_ratio: expansion ratio untuk FC internal
    """
    C = int(x.shape[-1])
    # 1) spatial downsample agar seq_len lebih kecil (misal 32x32 -> 8x8)
    pooled = AveragePooling2D(pool_size=pool_size, padding='same')(x)  # (B, H/p, W/p, C)
    Hp, Wp = int(pooled.shape[1]), int(pooled.shape[2])
    seq_len = Hp * Wp

    # 2) flatten spatial -> sequence
    seq = Reshape((seq_len, C))(pooled)  # (B, seq_len, C)

    # 3) channel expansion (MLP per-token)
    hidden_dim = int(C * mlp_ratio)
    y = Dense(hidden_dim, activation='gelu')(seq)  # (B, seq_len, hidden_dim)

    # 4) split -> u (value) & v (to be gated/mixed)
    # pastikan hidden_dim genap
    half = hidden_dim // 2
    u = Lambda(lambda z: z[:, :, :half])(y)
    v = Lambda(lambda z: z[:, :, half:])(y)

    # 5) SGU: token-mixing applied on v across the seq axis
    v = LayerNormalization(epsilon=1e-6)(v)

    # transpose to (B, channels, seq_len) so Dense can act on the seq axis when applied to last axis after transpose
    v_t = Lambda(lambda z: tf.transpose(z, perm=[0,2,1]))(v)  # (B, half, seq_len)

    # Dense across token axis: map seq_len -> seq_len (shared across channels)
    # This implements token mixing but with reduced seq_len so parameter size ok.
    v_t = Dense(seq_len, use_bias=True)(v_t)  # (B, half, seq_len)

    # transpose back to (B, seq_len, half)
    v_mixed = Lambda(lambda z: tf.transpose(z, perm=[0,2,1]))(v_t)

    # gating
    gated = Lambda(lambda args: args[0] * args[1])([u, v_mixed])  # (B, seq_len, half)

    # 6) project back to original channel dim
    proj = Dense(C)(gated)  # (B, seq_len, C)

    # 7) reshape back to spatial
    up = Reshape((Hp, Wp, C))(proj)  # (B, Hp, Wp, C)

    # 8) upsample to original spatial size
    up = UpSampling2D(size=(pool_size, pool_size), interpolation='bilinear')(up)  # (B, H, W, C)
    up = Conv2D(C, (1,1), padding='same', activation='relu')(up)
    up = BatchNormalization()(up)

    # 9) residual add (return same dim as input)
    out = Add()([x, up])
    return out

# === 3. Build Improved CNN Model (dengan gMLP terintegrasi) ===
def build_model():
    ...
    # --- Tambahkan gMLP block di sini (setelah bottleneck) ---
    # pool_size=4: 32x32 -> 8x8 (seq_len = 64) untuk membuat token-mixing lebih efisien
    x4 = gmlp_block(x4, pool_size=4, mlp_ratio=2)

    # Decoder dengan skip connections (tetap sama seperti semula)
...
if __name__ == "__main__":
    ...
    MODEL_SAVE_PATH = r"C:\KULIAH\TEKNIK SEMESTER 6\KERJA PRAKTEK\CNNDEHAZING\DATASET REVISI\MODEL REVISI\cnndehazing_SOTS8_MIX200_preprocessed_gmlp.h5"

    print("=== Pure CNN Dehazing dengan Advanced Preprocessing + gMLP block ===")
